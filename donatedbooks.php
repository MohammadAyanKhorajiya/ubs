...
$query = "SELECT b.*, c.category_name, 
          ...
          WHERE b.status = 'Available' AND b.type = 'Donated'";
...
<style>
.book-card img.fixed-img { height: 220px; object-fit: cover; }
.book-card h5 { font-size: 1rem; font-weight: 600; }
...
<div class="container my-5">
    ...
    <div class="filter-box mb-4 p-3 shadow-sm bg-white rounded">
        <form method="GET" class="row g-3 align-items-center">
            ...
            <div class="col-md-2 text-center">
                <button type="submit" class="btn btn-danger w-100 mt-4">Search</button>
            </div>
        </form>
    </div>
    ...
    <div class="book-row">
        <?php if(mysqli_num_rows($result) > 0) { ?>
                ...
                <div class="book-card">
                    <a href="bookdetails.php?id=<?= $book['book_id'] ?>" class="text-decoration-none text-dark">
                        <div class="card h-100 shadow-sm">
                            <img src="<?= htmlspecialchars($image) ?>" class="card-img-top fixed-img">
                            <div class="card-body p-3">
                                <h5 class="mb-1"><?= htmlspecialchars($book['title']) ?></h5>
                                <p class="text-muted text-small mb-1"><?= htmlspecialchars($book['category_name']) ?></p>
                                <p class="text-muted text-small mb-1">Condition: <?= htmlspecialchars($book['condition']) ?></p>
                                <p class="text-danger mb-0">Donated</p>
                            </div>
                        </div>
                    </a>
                </div>
            <?php } ?>
        <?php } else { ?>
            <p class="text-center text-muted mt-4">No donated books found.</p>
        <?php } ?>
    </div>
